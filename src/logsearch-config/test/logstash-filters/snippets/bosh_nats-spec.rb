...
describe "BOSH NATS healthcheck log parsing rules" do
  ...
  context "with monitor bosh-nats filter" do
    ...
    describe "when receiving logsearch logs" do
      when_parsing_log(
        '@type' => 'syslog',
        'syslog_program' => 'nats-to-syslog',
        '@message' => '{"Data":"{\"deployment\":\"p-logsearch-ff19a565962529d8131b\",\"job\":\"elasticsearch_master-partition-decfde1bc0cd88ca893a\",\"index\":0,\"job_state\":\"running\",\"vitals\":{\"cpu\":{\"sys\":\"0.0\",\"user\":\"0.0\",\"wait\":\"0.0\"},\"disk\":{\"ephemeral\":{\"inode_percent\":\"0\",\"percent\":\"1\"},\"persistent\":{\"inode_percent\":\"0\",\"percent\":\"0\"},\"system\":{\"inode_percent\":\"31\",\"percent\":\"42\"}},\"load\":[\"0.00\",\"0.01\",\"0.05\"],\"mem\":{\"kb\":\"3884812\",\"percent\":\"51\"},\"swap\":{\"kb\":\"0\",\"percent\":\"0\"}},\"node_id\":\"47e682bf-be6f-4a76-99d8-d9b3104bae51\"}","Reply":"","Subject":"hm.agent.heartbeat.8e244598-b1e2-4550-9b63-fa661df3a9f7"}'
      ) do

        it "sets the deployment to Logsearch" do
  ...
  context "with the default bosh-nats filter" do
    before(:all) do
      load_filters <<-CONFIG
        filter {
    ...
    describe "hm_agent_heartbeat logs" do
      when_parsing_log(
        ...
        it "sets @source.job" do
          expect(subject["@source"]["job"]).to eq "diego_cell-partition-7c53ed3ae2e7f5543b91"
        end
        ...
        it "sets @source.vm" do
          expect(subject["@source"]["vm"]).to eq "diego_cell-partition-7c53ed3ae2e7f5543b91/0"
        end
